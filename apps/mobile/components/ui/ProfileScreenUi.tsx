...
export const ProfileHero = ({
  ...
  return (
    <View className="pb-2">
      <View className="h-[300px] overflow-hidden bg-bg-elevated">
        <Image
          source={{ uri: coverUrl }}
        ...
        <TouchableOpacity
          className="absolute bottom-16 right-5 flex-row items-center gap-2 rounded-full border border-border bg-bg-card px-4 py-2.5"
          onPress={onEditCover}
          activeOpacity={0.85}
        >
          <Ionicons
          ...
          />
          <Text className="text-sm font-semibold text-text-primary">Cover</Text>
        </TouchableOpacity>
      </View>

      <View className="-mt-24 px-5">
        <View className="flex-row items-end justify-between">
          <View className="h-[132px] w-[132px] rounded-[38px] border border-border bg-bg p-1">
            {avatarUrl ? (
              <Image
                source={{ uri: avatarUrl }}
                className="h-full w-full rounded-[34px] bg-bg-elevated"
              />
            ) : (
              <View className="h-full w-full items-center justify-center rounded-[34px] bg-bg-elevated">
                <Ionicons
                  name="person"
                  size={52}
            ...
            <TouchableOpacity
              className="absolute -bottom-2 -right-2 h-11 w-11 items-center justify-center rounded-2xl border border-bg bg-primary"
          ...
          <TouchableOpacity
            className="mb-2 flex-row items-center gap-2 rounded-2xl bg-primary px-5 py-3"
            onPress={onEditProfile}
            activeOpacity={0.86}
          >
        ...
        <View className="mt-5">
          <View className="flex-row items-center gap-2">
            <Text className="max-w-[82%] text-[32px] font-bold text-text-primary">
              {displayName}
            </Text>
          ...
          <Text className="mt-1 text-base text-text-secondary">@{handle}</Text>
          ...
          <Text
            className="mt-5 text-lg leading-7 text-text-primary"
            numberOfLines={3}
          >
            {bio}
          </Text>
        ...
        >
          {interests.length ? (
            ...
            <TouchableOpacity
              className="h-12 flex-row items-center gap-2 rounded-2xl border border-border bg-bg-card px-4"
              onPress={onEditProfile}
              activeOpacity={0.85}
            >
              <Ionicons
          ...
          <TouchableOpacity
            className="h-12 w-12 items-center justify-center rounded-2xl border border-border bg-bg-card"
            onPress={onEditProfile}
            activeOpacity={0.85}
          >
...
export const ProfileJourneyCard = ({
  ...
  <View className="mx-5 mt-6 rounded-[28px] border border-border bg-bg-card p-5">
...
export const ProfileCompletionCard = ({
  ...
  <TouchableOpacity
    className="mx-5 mt-4 flex-row items-center rounded-[28px] border border-border bg-bg-card p-5"
    onPress={onPress}
    activeOpacity={0.88}
  >
    <View className="mr-4 h-14 w-14 items-center justify-center rounded-2xl bg-bg-elevated">
      <Ionicons name="heart" size={31} color={Colors.primaryLight} />
    </View>
    ...
    <View className="items-center rounded-2xl bg-primary px-4 py-3">
...
export const ProfileTabs = ({
  ...
  <View className="mx-5 mt-7 flex-row border-b border-border">
    <ProfileTabButton
      icon="grid-outline"
      label="Posts"
      count={counts?.posts}
...
export const ProfileEmptyState = ({
  ...
  <View className="items-center px-6 py-12">
    <View className="mb-4 h-[72px] w-[72px] items-center justify-center rounded-3xl border border-border bg-bg-card">
...
const IconButton = ({
  ...
  <TouchableOpacity
    className="h-11 w-11 items-center justify-center rounded-2xl border border-border bg-bg-card"
    onPress={onPress}
    activeOpacity={0.85}
  >
...
const JourneyStat = ({ icon, color, label, value, caption }: JourneyMetric) => (
  <View className="min-h-[132px] flex-1 basis-[47%] rounded-2xl border border-border bg-bg-elevated p-4">
    <View
    ...
    <Text className="mt-1 text-3xl font-bold text-text-primary">{value}</Text>
    <Text className="mt-1 text-xs text-text-muted">{caption}</Text>
  </View>
);
...
const AccountAction = ({
  ...
  <TouchableOpacity
    className="min-h-12 flex-1 flex-row items-center justify-center gap-2 rounded-2xl border border-border bg-bg-card"
    onPress={onPress}
    activeOpacity={0.85}
  >
    <Ionicons
...
const ProfileTabButton = ({
  ...
  >
      ...
      {count !== undefined ? (
        <View
          className={`min-w-5 items-center rounded-full px-1.5 py-0.5 ${
            active ? "bg-primary" : "bg-bg-elevated"
          }`}
        >
          <Text
    ...
    {active ? (
      <View className="absolute bottom-0 h-[3px] w-10 rounded-full bg-primary" />
    ) : null}
  </TouchableOpacity>
);
