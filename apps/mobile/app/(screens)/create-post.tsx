    ...
    createPostMutation.mutate(
      ...
      },
      {
        ...
        onError: (error: any) => {
          ...
          Alert.alert('Upload Failed', error?.response?.data?.message || 'Something went wrong while posting.');
        }
      }
    );
  };
  ...
  return (
    ...
    >
      ...
      <View className="flex-row items-center justify-between px-5 py-4">
        ...
        <Text className="text-white font-extrabold text-lg tracking-wide">Create Post</Text>
        ...
        >
          {isLoading ? (
            ...
            <Text className={`font-bold text-sm tracking-wide ${isPostDisabled ? 'text-[#666]' : 'text-black'}`}>
              Publish
            </Text>
          )}
        </TouchableOpacity>
      ...
      >
        <View className="flex-row items-center gap-1">
          ...
          <TouchableOpacity 
            className="w-12 h-12 rounded-full bg-[#1a1a1a] items-center justify-center active:opacity-60" 
            onPress={() => Alert.alert('Location', 'Location tagging coming soon!')}
          >
        ...
        <TouchableOpacity 
          ...
          disabled={isLoading}
        >
          <Ionicons name={isPublic ? "earth" : "people"} size={18} color={isPublic ? "#4A90E2" : "#FFF"} />
