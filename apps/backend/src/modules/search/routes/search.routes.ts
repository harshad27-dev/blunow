...
import { AuthRequest, authenticate } from '../../../common/middleware/auth.middleware';
...
class SearchController {
  ...
  getUnifiedSearch = async (req: Request, res: Response) => {
    try {
      ...
      const data = await this.repo.getUnifiedSearch(q, type, limit, (page - 1) * limit);
  ...
  getAdvancedSearch = async (req: Request, res: Response) => {
    try {
      const page = parseInt(req.query.page as string) || 1;
      const limit = parseInt(req.query.limit as string) || 20;
      const data = await this.repo.getAdvancedSearch(req.query, limit, (page - 1) * limit);
      res.json({ success: true, ...data });
    } catch (e: any) { res.status(500).json({ success: false, message: e.message }); }
  };
}
